<template>
  <div class="lifestyle-medical-form">
    <form @submit.prevent="isEditable ? saveChanges() : modify()">
      ...
      <br>
      <br>
      <button type="submit" style="margin-left:120px;" class="button-34">{{ isEditable ? 'Save Changes' : 'Modify' }}</button>
      <br>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveChanges() {
      if(this.formData.smokingStatus == null || this.formData.alcoholConsumption == null || this.formData.physicalActivityLevel == null || this.formData.stressLevel == null || this.formData.bodyMassIndex == null || this.formData.diabetes == null || this.formData.historyOfHD == null || this.formData.historyOfHA == null || this.formData.otherCondition == null){
        this.errors.message = 'Please complete all the fields before saving!';
      ...
      this.isEditable = false;
      this.formData.patientId = this.id;
      if (this.formData.historyOfHD === 'Other heart disease' && this.otherHeartDiseaseDetails) {
        this.formData.historyOfHD = this.otherHeartDiseaseDetails;
      }

      if(this.formData.otherCondition === 'Other' && this.otherConditionOption){
        this.formData.otherCondition = this.otherConditionOption;
      }


      PatientDetailsService.saveChanges(this.formData)
        .then(response => {
          console.log(response.data);
        })
        .catch(error => {
...
.error-message {
  ...
  text-align: center;
  box-shadow: 2px 4px 8px rgba(255, 1, 1, 0.05); /* Subtle shadow for depth */
  display: flex;
  align-items: center;
