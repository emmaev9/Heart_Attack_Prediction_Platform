<template>
  <div>
    <br>
    <button v-if="selectedVisit" class="button-link" @click="registerNewVisit">Register a new visit</button>
...
export default {
  ...
  data() {
    return {
      visits: [],
      selectedVisit: this.visitDefault
    };
  },
  props: {
    id: {
  ...
  methods:{
    ...
    registerNewVisit() {
      this.$emit('register-selected');
      this.$router.push({ name: 'PatientRegisterVisit', params: {id: this.selectedVisit.patientId }} );
