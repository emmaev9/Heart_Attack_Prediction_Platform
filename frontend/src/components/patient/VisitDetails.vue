<template>
  ...
  <div class="button-container">
    <Button v-if="isEditable && !details" label="Save visit"  icon="pi pi-check" type="submit" @click="this.saveChanges()" class="button-34" />
...
export default {
  ...
  data() {
    return {
      ...
      doctors: [],
      doctorOfPatient: {},
      selectedDoctorId: { id: null, label: '' },
      visit: {
        visitDate: '',
        doctorId: '',
        patientId: '',
  ...
  watch: {
    '$route.params.visitId'(newVisitId) {
      if (newVisitId) {
        this.getSelectedVisit(newVisitId);
      }
    },
  },
  methods: {
    ...
    saveChanges(){
      const visitToSave = {
        ...
        VisitService.updateVisit(visitToSave)
          .then(response => {
            console.log(response.data);
    ...
    getSelectedVisit(){
      ...
      VisitService.getVisit(this.visitId)
        .then(response => {
          ...
          this.visit.visitDate = this.formatDateForBackend(this.visit.visitDate);
          this.visit.trestbps = this.response.data.trestbps;
          this.isEditable = false;
          this.isEditableDate=false;
          this.isEditableReason=false;
          this.isEditableTrestbps=false;
  ...
  computed: {
    formattedVisitDate: {
      get() {
        ...
        return this.visit.visitDate;
      },
      set(newValue) {
        this.visit.visitDate = newValue;
      }
    }
  },
  ...
  mounted() {
    ...
    if (this.$route.params.visitId) {
      this.getSelectedVisit(this.$route.params.visitId);
    }
  }
}
